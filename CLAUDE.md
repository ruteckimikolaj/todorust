...
**Quick-add & reschedule (Phase 4).** New-task capture accepts one-line natural-language tokens through `parse_quick_add`: `@project`, `!priority` (`1`/`l`/`low` … `3`/`h`/`high`), `^date`, and `%recurrence`. The input block's title turns into a live parse preview so users see what will be applied on submit. Reschedule presets `t` (today), `T` (tomorrow), `w` (next week) act on the highlighted task via `UiState::reschedule_active`, which preserves the task's original time-of-day when it was a real user-set time (not the placeholder 09:00 or 23:59). `r` opens the `Rescheduling` prompt sharing `parse_date_shortcut` — the same vocabulary used by quick-add `^`.

**Recurring tasks (Phase 5).** `Recurrence` (in `app/mod.rs`) is `EveryDays(u32)` / `EveryWeeks(u32)` / `EveryMonths(u32)` / `Weekly(chrono::Weekday)`. Each variant serialises via `to_shortcut` (`2d`, `3w`, `1m`, `mon`), rendered by `title` (`"every 2 weeks"`), and advances via `next_after(from)` — `Weekly` always jumps ≥7 days so completing "every Mon" on Monday moves to next Monday, and `EveryMonths` uses `checked_add_months` so Jan 31 → Feb 29. `Task.recurrence: Option<Recurrence>` is set via the `%` quick-add token or the *Repeat* row in the edit sheet (parsed by `parse_recurrence`, re-exported from `app::mod`). `App::complete_active_task` delegates to `spawn_next_occurrence`, which inserts a fresh copy right after the completed task with reset subtask flags, a new uuid, `due_date = next_after(previous_due or now)`, and moves the cursor to it. The task list shows a `↻` glyph on recurring rows. DB persistence: `recurrence TEXT` column, additive ALTER migration in `init_schema`; `save_tasks` writes the shortcut, `load_tasks` runs it back through `parse_recurrence`.

**Phase 6 polish.** Statistics gained a **Review** section (completion rate, avg open-task age, this-vs-last-week completion trend, top three projects by open count) between the summary/chart row and the completed list. Task rows swapped the inline priority glyph for a coloured left **gutter bar** (`▍`) rendered in the priority colour; the glyph still appears in the details view and the quick-add preview. `todorust export <path>` / `todorust import <path> [--replace]` **CLI subcommands** serialise the task store as JSON via serde (use `-` for stdin/stdout); imports append by default. **Mouse-wheel scroll** is routed through `handle_mouse_event` in Normal mode to reuse the previous/next-task helpers. **Bulk actions** live on a marked-uuid set (`v` toggles, `Shift+V` clears); when the set is non-empty, Space/x, d/Del, and 1/2/3 fan out via `App::bulk_complete` / `bulk_delete` / `bulk_set_priority` instead of hitting the active task alone, and the Controls bar title shows `"— N selected"`.
